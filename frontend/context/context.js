...
export const ChaingariProvider = ({ children }) => {
  const [hasPhantom, setHasPhantom] = useState(true)
  const [connected, setConnected] = useState(false)
  const [phantom, setPhantom] = useState()
  const [tiktoks, setChaingari] = useState([])

  useEffect(() => {
    ;(async () => {
      const data = await getChaingari()

      setChaingari(data)
    })()
    console.log(Chaingari)
  }, [])

  const getChaingari = async () => {
